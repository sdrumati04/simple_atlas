...
- In-game configuration is managed by `SimpleAtlasConfigManager` / `SimpleAtlasConfig` (`maxAtlasMapCount`, `maxWaypoints`, `bannerWaypointsOnly`, `consumePaperForHigherScales`, `waypointIconSize`, `playerIconSize`), with GUI integration via `SimpleAtlasConfigScreen` (Cloth Config) and `SimpleAtlasModMenuIntegration` (ModMenu).
- Atlas state is stored in `ModComponents.ATLAS_CONTENTS` using `component/AtlasContents.java`.
- `AtlasContents` stores map IDs + waypoint state (`waypoints`, `selectedWaypointIconIndex`, `nextWaypointNumber`), `selectedScale`, active `blankMapCount`, `paperCount`, enforces configured map/waypoint caps dynamically, sanitizes waypoint names/icon indices/dimensions, and retains legacy `sub_map_ids` for codec backwards compatibility.
- Core gameplay logic is in `item/AtlasItem.java`:
  ...
  - `use` auto-creates initial map if atlas has blank maps but 0 maps, groups maps by dimension and scale, builds layout via `AtlasLayoutBuilder.build(...)`, sends `OpenAtlasScreenPayload`, and registers active viewers in `AtlasViewManager`. When atlas has 0 maps and cannot auto-create, sends an actionbar overlay warning (`message.simple_atlas.no_maps_inserted`) with fail sound.
  - `inventoryTick` keeps atlas `DataComponents.MAP_ID` synced with current position and selected scale (`AtlasMapSelector`), delegates to `Items.FILLED_MAP.inventoryTick(...)` for vanilla marker behavior, and performs multi-scale live exploration. When equipped in hand or when a new map tile is auto-created during exploration, briefly displays active scale, maps count, empty maps count, and paper count on the actionbar overlay (avoiding re-display on routine tile boundary switches between already-explored maps). Vanilla map tooltip metadata (`#id`, scale, level) is suppressed via `DataComponents.TOOLTIP_DISPLAY` with `shows(MAP_ID) == false`, showing only pure atlas information. When walking outside existing map bounds, if the atlas has blank maps (`blankMapCount > 0` or creative) and sufficient paper for higher scales when `consumePaperForHigherScales` is enabled, it automatically consumes one blank map and required paper, generates the new grid-aligned map tile, plays `SoundEvents.UI_CARTOGRAPHY_TABLE_TAKE_RESULT`, syncs Remapped packets if loaded, and continues mapping seamlessly. If blank maps or paper are missing or map cap is reached, throttled actionbar overlay warnings (`no_empty_maps`, `not_enough_paper`, `map_limit_reached`) are displayed with fail sound.
  - `appendHoverText` displays active scale ratio (`1:X`), all contained scale ratios, available empty maps count (`blankMapCount`), and available paper count (`paperCount`).
...
- Crafting recipe (`recipe/AtlasAddRecipe.java`):
  - Special crafting recipe allowing players to insert blank maps (vanilla `Items.MAP` and Remapped `remapped:empty_map`), paper (`Items.PAPER`), as well as filled maps directly into the atlas inside the 2x2 player crafting inventory or 3x3 crafting table.
- Cartography behavior is mixin-driven (`CartographyTableMenuMixin`, `CartographyTableAdditionalSlotMixin`, `CartographyTableMapSlotMixin`, `CartographyTableResultSlotMixin`):
  ...
  - Atlas + paper: scale atlas maps up by +1 in-place using `AtlasCartographyScaler.scaleAtlas`, or add paper to `paperCount` if atlas cannot be scaled up.
  ...
  - Atlas + atlas: merge map/waypoint contents and sum `blankMapCount` and `paperCount` (multi-scale supported) when total map count does not exceed limit.
